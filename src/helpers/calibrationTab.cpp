...
void CalibrationTab::init() {
   ...
   checkBottomImage = new QCheckBox(
                  "Switch to Bottom Camera", this);
   checkBottomImage->setChecked(false);
   checkboxLayout->addWidget(checkBottomImage);
...
#endif
      QStringList items;
      items << tr("scout.local") << tr("heavy.local") << tr("demoman.local") <<
...
void CalibrationTab::redraw() {
   ...
   if (checkBottomImage->isChecked()) {
      currentFrame = botFrame;
   } else {
      currentFrame = topFrame;
   }
/*
   std::cout << "checkBot = " << checkBottomImage->isChecked() <<
                " currentFrame = " << currentFrame << " topFrame = " <<
                topFrame << " botFrame = " << botFrame << std::endl;
*/
   if (currentFrame) {
